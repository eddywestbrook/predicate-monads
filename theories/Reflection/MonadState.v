...
Class MonadState M `{MonadStateOps M} : Prop :=
  {
    ...
    monad_state_get :
      forall A `{OType A} (m : M A _ _) prp,
        bindM @o@ getM @o@ (ofun (fun _ => m) (prp:=prp)) =o= m;

    monad_state_get_put :
      forall A `{OType A} (f : St -o> unit -o> M A _ _) prp1 prp2,
        bindM @o@ getM @o@
    ...
    monad_state_put_get :
      forall A `{OType A} s (f : unit -o> St -o> M A _ _) prp1 prp2,
        bindM @o@ (putM @o@ s) @o@
    ...
    monad_state_put_put :
      forall A `{OType A} s1 s2 (f : unit -o> unit -o> M A _ _) prp,
        bindM @o@ (putM @o@ s1) @o@
