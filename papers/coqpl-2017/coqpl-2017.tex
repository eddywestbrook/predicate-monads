...
universal quantification, existential quantification, and implication. The
fourth operation, $\singleP$, builds the singleton predicate, i.e., the least
predicate containing a given monadic computation. The final operation is the
entailment pre-order over predicates. Intuitively, $P_1\vdash P_2$ means that
any computations which satisfy $P_1$ will always satisfy $P_2$. We require that
the equivalence $\approx$ over $P_M$ be defined as the symmetric intersection of
$\vdash$, i.e., such that $P_1\approx P_2$ iff $P_1\vdash P_2$ and
$P_2\vdash P_1$. Using the final two operations above, we can then define
\[
...
FIXME HERE: predicate monad laws
\begin{itemize}
\item $\singleP$ applied to each operation yields the corresponding operation in
  $P_M$; same with effectful operations such as get and put
\item Introduction and elimination laws (just give examples for forall)
\end{itemize}


FIXME HERE: describe the set predicate monad, and explain that
transformers can be used to build up predicate monads
